...
def evaluate_model(label_folder, output_folder):
    # Load the labels.
    patient_ids = find_data_folders(output_folder)
    ...
    plot_roc_curve(label_outcomes, output_outcome_probabilities)
...
def plot_roc_curve(labels, outputs):
    ...
    #idx = np.argwhere(fpr==0.05)[0][0]
    plt.title("ROC from validation on the training set")
    plt.hlines(0.05, xmin = 0, xmax = 0.05, color="red",linestyle='--')
    plt.vlines(0.05, ymin = 0, ymax = 0.05, color="red",linestyle='--')
    plt.xlim(0,1)
    plt.ylim(0,1)
    plt.savefig("./roc_plt.png",dpi=300, bbox_inches='tight')
    #plt.show()
